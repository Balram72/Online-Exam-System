			...
			else
			{
				...
				foreach($result as $row)
				{
					if($row['student_email_verified'] == 'Yes')
					{
						...
						{
							$error = 'Sorry, Your account has been disable, contact Admin';
						}
					}
					else
					{
						$error = 'You have not verify you email address, so for email verification, click <a href="resend_email_verification.php">here</a>';
					}
				}
			}
	...
	if($_POST['page'] == 'exam')
	{
		if($_POST["action"] == 'fetch')
		{
			...
			foreach($result as $row)
			{
				...
				if(is_null($row['exam_result_datetime']))
				{
				...
				if($total_subject > 0)
				{
					...
					else
					{
						if(time() > $exam_last_subject_end_datetime)
						{
							...
							$status = '<span class="badge bg-dark">Completed</span>';
				...
				$sub_array[] = '<a href="instructions.php?ec='.$row["exam_code"].'" '.$hidde.'>View</a>';
		...
		if($_POST['page'] == 'view_subject_exam')
		{
			if($_POST['page'] == 'view_subject_exam') {
				if($_POST['action'] == 'load_question') {
						...
						foreach($result as $row) {
								// Get all questions for numbering
								$object->query = "
										SELECT exam_subject_question_id, exam_subject_question_title 
										FROM exam_subject_question_soes 
										WHERE exam_id = '".$_POST["exam_id"]."' 
										AND exam_subject_id = '".$_POST["exam_subject_id"]."' 
										ORDER BY exam_subject_question_id ASC
								";
								$all_questions = $object->get_result();
		
								// Find the question number
								$question_number = 1;
								foreach ($all_questions as $question) {
										if ($question['exam_subject_question_id'] == $row['exam_subject_question_id']) {
												break;
										}
										$question_number++;
								}
								...
								$output .= '
										...
										<b>Question No:-&nbsp;' . $question_number . '</b> ' . $row["exam_subject_question_title"] . '
										<div class="card-body">
												<div class="row">
								';
								...
								$output .= '
										...
										<div class="row justify-content-between">
												<div class="col-auto">
														<button type="button" name="previous" class="btn btn-info btn-lg previous" id="' . $previous_id . '" ' . $if_previous_disable . ' style="color:#333333; background-color:#92c4f2; border:none;opacity: 1.65;">Previous</button>
												</div>
												<div class="col-auto text-right">
														<button type="button" name="next" class="btn btn-warning btn-lg next" id="' . $next_id . '" ' . $if_next_disable . ' style="background-color:#1ca7c0; color:white; border:none;">Save & Next</button>
												</div>
										</div>
								';
						}
		...
		if($_POST['action'] == 'question_navigation')
		{
			...
			$output = '
				 ...
				 <p style="text-align:center"><b>Question Section</b></p>
				 <hr/>
					<div class="row">
			';
			$count = 1;
			...
			{
				$output .= '
					<button type="button" class="btn btn-light bg-body border border-secondary text-black question_navigation  fixed-size-button" data-question_id="'.$row["exam_subject_question_id"].'" style="width: 40px; height:45px;" >'.$count.'</button>
				';
				$count++;
			}
			$output .= '</div>';
